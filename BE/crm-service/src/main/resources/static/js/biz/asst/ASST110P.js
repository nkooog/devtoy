...
StringBuffer.prototype.size = function() {
	return this.buffer[this.buffer.length];
};
...
$(document).ready(function (){
	...
	buffer_message = new StringBuffer();
	assistantConnect();
	mecabConnect();
});
...
function assistantConnect() {
	...
	assistantWebSocket = new WebSocket('ws://127.0.0.1:18090');
		...
		switch (commandType) {
			...
			case 2:
				...
				if(buffer || buffer.size() == 0) {
					if (type == "Rx") {
						buffer.append('<div class="item"><div class="chatbox"><p class="msg"><span id="Rx" class="removeElement"></span>');
					}else{
						buffer.append('<div class="item mymsg"><div class="chatbox"><p class="msg"><span id="Tx" class="removeElement"></span>');
					}
					buffer.append('</p><span class="time removeElement" id="chatTime">');
					buffer.append('</span></div></div>');
				}

				if(isValue) {
					$("#CNSL151P_ChatBox").append(buffer.toString());
					isValue = false;
				}

				if(data["Type"] != null && data["Type"] !== 'undefined') {
					document.querySelector('#'+type).innerHTML = buffer_message.toString();
				}

				if(JSON.parse(isEpd.toLowerCase())) {
					document.querySelector('#'+type).innerHTML = message;
					document.getElementById('chatTime').append(formattedTime);
					document.querySelectorAll('.removeElement').forEach( e => {
						e.removeAttribute('id');
					});

					buffer.remove();
					buffer_message.remove();
					isValue = true;
				}
...
//		$("#treeKMST300P").data("kendoTreeView").dataSource.data(resultTree);
		...
		if(list.length > 0) {
			...
			Utils.ajaxSyncCall("/kmst/KMST210SEL02", JSON.stringify(param), function(data){
				...
				console.log(response.kmst211VoList);
...
function KMST300P_fnSearchResult(data){
	...
	if(item.kmstResult.hits.hits[0]) {
		...
		$("#ASST100P_test").html(item.kmstResult.hits.hits[0].highlight["mokti.moktiCttTxt"]);
	}
...
function KMST300P_fnListDataSet(list,tree,src,type){
	...
	for(let item of list){
		...
		let txt ;
		if(item.highlight["mokti.moktiCttTxt"] != null) {
			txt = item.highlight["mokti.moktiCttTxt"].length == 1 ? item.highlight["mokti.moktiCttTxt"][0] : item.highlight["mokti.moktiCttTxt"].join(" ")
		}else {
			txt = "";
		}
		...
		let data = {
			...
			title: item.highlight.title == null ? item._source.title : item.highlight.title[0],
			moktiCtt: txt,
