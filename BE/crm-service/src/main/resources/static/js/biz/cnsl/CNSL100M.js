...
var tabFrmId;
var tabFrmCnslId;
var tabFrmCnslHistId;
var tabFrmCnslCallId;	//kw---20240415 : 통화이력탭 frmID 구하기
var tabFrmCnslInfoId;	//kw---20240425 : 1차버전 부재중 시 상담이력 초기화 - 고객상담 frmId 구하기
...
function getFrmId() {
	...
	$(".tabCNSL100M").find("li").find("span").each(function() {
    	let tabCNSL100M_url  = String($(this).attr("data-content-url"));
    	
    	if (tabCNSL100M_url.indexOf('112T') !== -1) {
    		tabFrmId = tabCNSL100M_url.substring(tabCNSL100M_url.length - 8, tabCNSL100M_url.length - 4);
    	}
    	
    	if (tabCNSL100M_url.indexOf('110T') !== -1) {
    		tabFrmCnslId = "110T"
    	} else if (tabCNSL100M_url.indexOf('160T') !== -1) {
    		tabFrmCnslId = "160T"
    	}
    	
    	if (tabCNSL100M_url.indexOf('113T') !== -1) {
    		tabFrmCnslHistId = "113T"
    	} else if (tabCNSL100M_url.indexOf('213T') !== -1) {
    		tabFrmCnslHistId = "213T"
    	}
    	
    	//kw---20240415 : 통화이력탭 frmID 구하기
    	if (tabCNSL100M_url.indexOf('CNSL101T') !== -1) {
    		tabFrmCnslCallId = "101T";
    	} else if (tabCNSL100M_url.indexOf('CNSL201T') !== -1) {
    		tabFrmCnslCallId = "201T";
    	}
    	
    	//kw---20240425 : 1차버전 부재중 시 상담이력 초기화 - 고객상담 frmId 구하기
    	if (tabCNSL100M_url.indexOf('112T') !== -1) {
    		tabFrmCnslInfoId = "112";
    	} else if (tabCNSL100M_url.indexOf('162T') !== -1) {
    		tabFrmCnslInfoId = "162";
    	}
    	
	});
	
	if(Utils.isNull(tabFrmId)){
		tabFrmId = "CNSL";
	}
}
...
$(document).ready(function () {	
	...
	CNSL100M_createPatInfoGrid();	//kw---20240213 : 전화받기 팝업창 옆에 고객정보 표출
    ...
    var CNSL100M_TabNum = $("#CNSL100M_GridNum").val();
    var CNSL100M_TabKey = $("#CNSL100M_GridKey").val();
    CNSL100M_fnGetPoto();
    CNSL100_tabLoad();
    heightResizeCNSL();
	$(window).on({ 
	...
	if( Utils.isNotNull(CNSL100M_TabNum) && Utils.isNotNull(CNSL100M_TabKey) ){
		setTimeout(function() {
			CNSL100MTabSel(CNSL100M_TabNum, CNSL100M_TabKey);
		}, 500);
    }
    ...
    $("#modalGridPatInfo tbody tr").on("dblclick", function() {
    	let myVar = modalGridPatInfo[0].dataItem($(this).closest("tr"));
    });
...
function heightResizeCNSL() {         
	...
	$(".tabCNSL100M").find('#grdCNSL110T .k-grid-content').css('height', screenHeight-250);
	$(".tabCNSL100M").find('#grdCNSL160T .k-grid-content').css('height', screenHeight-250);
	$('.cnslSubGrid').find('.k-grid-content').css('height', screenHeight-672);
	$('.cnslSubGrid2').find('.k-grid-content').css('height', screenHeight-704);
	$('.cnslSubGrid3').find('.k-grid-content').css('height', screenHeight-612);
	$('.cnslSubGrid4').find('.k-grid-content').css('height', screenHeight-644);

	// 가운데 토글버튼 화면사이즈 변환시 사라지는 현상 임시 수정 ---- (S)
	$(".btVerticalExtend").removeClass('Contracts');
	$(".CNSL112.csBasicInfo").css('height', '333px');
	$(".btVerticalExtend").css('top', '390px');
	// 가운데 토글버튼 화면사이즈 변환시 사라지는 현상 임시 수정 ---- (E)
//	$('.CNSL113 #cnslCtt').css('height', screenHeight-660);
	$('.CNSL113 #cnslCtt').css('height', screenHeight-625);
	// 우측 컨텐츠 Height 체크~!
	// $('#tabCNSL100M_3 .templateCardDiv').css('height', screenHeight-130);
	// $('#tabCNSL100M_3 .templateCardDiv').css('overflow-y', 'auto');
...
function counselAllInit() {
	...
	custId = '';
	custNm = '';
	if ( typeof(custInitCNSL112) != 'undefined' ) {
		custInitCNSL112();
	}
	counselInit();
}
...
function CNSL100MTabSel(tabIndex, key)  {
	switch (tabIndex) {
	    case "1":
	    	CNSL100MTabClick("/bcs/cnsl/CNSL101T");
	    	if ( typeof(setCNSL101SEL01) == 'undefined' ) {
	    		setTimeout(function() {
	    			CNSL100MTabSel(tabIndex, key)
	    		}, 500);
	    	}else {
	    		setCNSL101SEL01(key);
	    	}
	        break;
	    case "2":
	    	CNSL100MTabClick("/bcs/cnsl/CNSL102T");
	        ...
	        break;
	    case "3":
	    	CNSL100MTabClick("/bcs/cnsl/CNSL103T");
	        ...
	        break;
	    case "4":
	    	CNSL100MTabClick("/bcs/cnsl/CNSL104T");
...
function CNSL100MTabClick(tabId){
	$(".tabCNSL100M").find("li").find("span").each(function() {
...
function dateTimeMMCNSL100(str) {
...
function dateTimeSSCNSL100(str) {
	if ( typeof str == 'undefined') {
		return ''
	}else {
		if (str.length != 14) {
...
function btVerticalExtend(obj) {
	...
	if ($(obj).hasClass("Contracts")){
		...
		$(".CNSL112.csBasicInfo").css('height', screenHeight-403);
...
//kw---20240213 : 전화받기 팝업창 옆에 고객정보 표출
//kw---20240213 : 그리드 생성
function CNSL100M_createPatInfoGrid(){
...
//kw---20240510 : 아웃바운드, 인바운드시 환자조회 팝업창에(v2) 그리드 항목을 추가
function CNSL100M_fnPatInfoGridAddColumn(){
	if(modalGridPatGridEmr == true){
...
//kw---20240222 : 고객정보 더블 클릭시 선택된 아이템으로 고객 정보 셋팅2
...
//kw---20240213 : 고객정보 그리드에 데이터 넣기
function srchCNSLCustInfo(telNo){
	...
	if(funcType == "H"){
		...
		Utils.ajaxCall('/cnsl/CNSL110SEL01', CNSL110T_jsonStr, function(data) {
			...
			modalGridPatInfo[0].dataSource.data(arrReturn);
			
			//kw---20240228 : 픽업창이 닫히고 데이터가 들어왔을 경우
			if(modalGridPatInfo[0].dataSource.data().length <= 1){
				var modalElement = $("#modalAlertCall");
				var isModalVisible = modalElement.is(":visible");
				if (!isModalVisible) {
					var dataItem = modalGridPatInfo[0].dataSource.data()[0];
				    if ( window['set' + tabFrmId + 'CustInfo'] ) {
				    	new Function ( 'set' + tabFrmId + 'CustInfo(' + JSON.stringify(dataItem) +')')();
					}
				    CNSL126P_fnModalClose();
			...
			kendo.ui.progress($("#modalGridPatInfo"), false);
...
//kw---20240409 : 아웃바운드 시 고객목록 팝업 추가
//kw---20240222 : 고객정보 더블 클릭시 선택된 아이템으로 고객 정보 셋팅1
function CNSL100M_confirm_outBound(e){
	var selectedRow = $(e.target).closest("tr");
...
function CNSL100M_onlyNumber(event) {

	const input = event.target;
	// 입력 값에서 숫자가 아닌 문자를 제거
