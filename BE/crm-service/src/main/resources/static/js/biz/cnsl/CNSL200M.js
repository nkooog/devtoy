...
function getFrmId() {
	...
	$(".tabCNSL200M").find("li").find("span").each(function() {
    	let tabCNSL200M_url  = String($(this).attr("data-content-url"));
    	
		if (tabCNSL200M_url.indexOf('112T') !== -1 ) {
			tabFrmId = tabCNSL200M_url.substring(tabCNSL200M_url.length - 8, tabCNSL200M_url.length - 4);
		}
		
		if (tabCNSL200M_url.indexOf('110T') !== -1) {
    		tabFrmCnslId = "110T"
    	} else if (tabCNSL200M_url.indexOf('160T') !== -1) {
    		tabFrmCnslId = "160T"
    	}
		
		//kw---20240415 : 통화이력탭 frmID 구하기
    	if (tabCNSL200M_url.indexOf('CNSL101T') !== -1) {
    		tabFrmCnslCallId = "101T";
    	} else if (tabCNSL200M_url.indexOf('CNSL201T') !== -1) {
...
$(document).ready(function () {
	...
	CNSL200M_createPatInfoGrid();	//kw---20240213 : 전화받기 팝업창 옆에 고객정보 표출
	CNSL200M_createPatInfoGrid_outBound();
    ...
    var CNSL200M_TabNum = $("#modalGridPatInfoNum").val();
    var CNSL200M_TabKey = $("#modalGridPatInfoKey").val();
    CNSL200M_fnGetPoto();
    CNSL200_tabLoad();
    heightResizeCNSL();
	$(window).on({ 
	...
	if( Utils.isNotNull(CNSL200M_TabNum) && Utils.isNotNull(CNSL200M_TabKey) ){
		setTimeout(function() {
			CNSL200MTabSel(CNSL200M_TabNum, CNSL200M_TabKey);
		}, 500);
    }
...
function get112() {
	$(".tabCNSL200M").find("li").find("span").each(function() {
    	let tabCNSL200M_url  = String($(this).attr("data-content-url"));
		if (tabCNSL200M_url.indexOf('112T') !== -1 ) {
			url112 = tabCNSL200M_url;
			id112 = tabCNSL200M_url.substring(tabCNSL200M_url.length - 8, tabCNSL200M_url.length - 1);
		}
	})
	if ( url112 == '' ) {
		setTimeout(get112, 200);
	}
}
...
function heightResizeCNSL() {        
	...
	$(".tabCNSL200M").find('#grdCNSL210T .k-grid-content').css('height', screenHeight-250);
	// mhlee 7/31 사이즈 조정
...
//	$('.CNSL213 #cnslCtt').css('height', scre/enHeight-600);
	...
	// 우측 컨텐츠 Height 체크~!
	// $('#tabCNSL200M_3 .templateCardDiv').css('height', screenHeight-130);
	// $('#tabCNSL200M_3 .templateCardDiv').css('overflow-y', 'auto');
...
function CNSL200MTabSel(tabIndex, key)  {
	switch (tabIndex) {
	    case "1":
	        ...
	        break;
	    case "2":
	    	CNSL200MTabClick("/bcs/cnsl/CNSL202T");
	        ...
	        break;
	    case "3":
	    	CNSL200MTabClick("/bcs/cnsl/CNSL203T");
	        ...
	        break;
	    case "4":
	    	CNSL200MTabClick("/bcs/cnsl/CNSL204T");
...
function CNSL200MTabClick(tabId){
	$(".tabCNSL200M").find("li").find("span").each(function() {
...
// 사진조회
function CNSL200M_fnGetPoto(){
	if(GLOBAL.session.user.potoImgIdxFileNm != ""){
		$(".img_bx > img").attr({ src: GLOBAL.contextPath + "/photo/" +  GLOBAL.session.user.tenantId + "/" + GLOBAL.session.user.potoImgIdxFileNm});
		var img = document.querySelector('.img_bx > img');
		
		var url_ststus = fnUrlExists(img.src);
		if(url_ststus == 404 || url_ststus == '404'){
			$(".img_bx > img").attr({ src: GLOBAL.contextPath + "/images/contents/person.png" });
		}else{
			$(".img_bx > img").attr({ src: GLOBAL.contextPath + "s/photo/" +  GLOBAL.session.user.tenantId + "/" + GLOBAL.session.user.potoImgIdxFileNm});
		}		
	}else{
		$(".img_bx > img").attr({ src: GLOBAL.contextPath + "/images/contents/person.png" });
	}
...
function dateTimeMMCNSL200(str) {
...
function dateTimeSSCNSL200(str) {
	if ( typeof str == 'undefined') {
		return ''
	}else {
		if (str.length != 14) {
...
//kw---20240213 : 전화받기 팝업창 옆에 고객정보 표출
//kw---20240213 : 그리드 생성
function CNSL200M_createPatInfoGrid(){
...
//kw---20240409 : 아웃바운드 시 고객목록 팝업 추가
function CNSL200M_createPatInfoGrid_outBound(){
...
//kw---20240510 : 아웃바운드, 인바운드시 환자조회 팝업창에(v2) 그리드 항목을 추가
function CNSL200M_fnPatInfoGridAddColumn(){
	if(modalGridPatGridEmr == true){
...
$("#modalGridPatInfo_outBound").on('dblclick','tbody tr[data-uid]',function (e) {
	CNSL200M_confirm_outBound(e);
});

//kw---20240409 : 아웃바운드 시 고객목록 팝업 추가
//kw---20240222 : 고객정보 더블 클릭시 선택된 아이템으로 고객 정보 셋팅1
function CNSL200M_confirm_outBound(e){
	var selectedRow = $(e.target).closest("tr");
