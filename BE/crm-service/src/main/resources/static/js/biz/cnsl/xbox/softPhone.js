...
function alertingView(flag, data) {
	if (flag == 'hide') {
		...
		/*if(!Utils.isNull(data.remote)){
			...
			Utils.ajaxCall("/cnsl/CNSL100SEL02", JSON.stringify(CNSL100_Param), function (result) {
		...
		Utils.ajaxCall("/cnsl/CNSL100SEL01", CNSL100_jsonStr, CNSL100SEL01);
...
//		};
//		var CNSL100_jsonStr = JSON.stringify(CNSL100_data);
//		Utils.ajaxCall("/cnsl/CNSL110SEL01", CNSL100_jsonStr, CNSL110SEL01);
...
function CNSL100SEL01(data) {
	if ( data.CNSL100SEL01 != null ) {
		$(".phoneFrame #serviceName").html(data.CNSL100SEL01);
	}
}

function CNSL110SEL01(data) {
	var CNSL110SEL01Index =  JSON.parse(data.CNSL110SEL01);
	if ( CNSL110SEL01Index.length == 1) {
		$(".phoneFrame #custId").html(CNSL110SEL01Index[0].custId);
	} 
	modalCNSL126P();
}
...
function receivePhonePickup() {
	...
	if(mode == 'factory'){	//kw---20240920 : 시연용 - 전화시뮬레이션 추가
