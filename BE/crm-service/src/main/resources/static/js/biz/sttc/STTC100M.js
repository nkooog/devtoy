var startSTTC100M, endSTTC100M;
var STTC100M_CommCodeList;
var STTC100M_userInfo = GLOBAL.session.user;
var STTC100M_CommCodeList;
var STTC100M_Grid =new Array(1);
var STTC100M_Chart =new Array(1);

var STTC100M_cmmCodeList, STTC100M_grdSTTC100M, STTC100MDataSource, STTC100M_ToolTip;
...
$(document).ready(function() {
	...
	Utils.ajaxCall('/comm/COMM100SEL01',  JSON.stringify({ "codeList": mgntItemCdList}),function(data){
		STTC100M_CommCodeList = JSON.parse(JSON.parse(JSON.stringify(data.codeList)));
		Utils.setKendoComboBox(STTC100M_CommCodeList, "S0013", '#STTC100M input[name=STTC100M_searchType]',"DY",false);		//조회유형
		Utils.setKendoComboBox(STTC100M_CommCodeList, "S0014", '#STTC100M input[name=STTC100M_statType]',"",false);			//통계유형
		Utils.setKendoComboBox(STTC100M_CommCodeList, "S0015", '#STTC100M input[name=STTC100M_searchCallType]',"","전체");	//콜유형
	},null,STTC100M_fnSearchList,null );
	...
	STTC100MDataSource = {
		schema: {
			...
			model: {
				fields: {
					userNm			:	{ field: "userNm", 			type: "string", editable: false},
	...
	STTC100M_grdSTTC100M = $("#grdSTTC100M").kendoGrid({
        dataSource: STTC100MDataSource,
        autoBind: false,
        resizable: true,
        refresh: true,
        sortable: false,
        sort: function (e) {
            if (STTC100M_grdSTTC100M.dataSource.total() === 0) {
                e.preventDefault();
            } 
        },
        ...
        columns: [
        	{ width : 130, field : "userNm", title : STTC100M_langMap.get("STTC100M.gridTitleCnslName"), attributes:{ style:"text-align:left"}, locked: true},
        	{ width : 100, field : "date0", title : $("#STTC100M_startDate").val(), attributes:{ style:"text-align:right"},},
    		{ width : 100, field : "totalCount", title : STTC100M_langMap.get("STTC100M.gridTitleTotal"), attributes:{ style:"text-align:right"}},
        ],
        dataBound: function(e) {
            ...
            for (var j = 0; j < dataItems.length; j++) {
              ...
              var discontinued = dataItems[j].get("userNm");
        ...
        noRecords: { template: '<div class="nodataMsg"><p>'+STTC100M_langMap.get("grid.nodatafound")+'</p></div>' },
    }).data("kendoGrid");
	
	//1.테넌트 설정
	CMMN_SEARCH_TENANT["STTC100M"].fnInit(null,STTC100M_tenatIdSet);
...
//1. 시작 데이터 피크 이벤트
function startChangeSTTC100M() {
	var startDate = startSTTC100M.value();
	var endDate = endSTTC100M.value();
	
	if (startDate) {
		startDate = new Date(startDate);
		startDate.setDate(startDate.getDate());
		endSTTC100M.min(startDate);
	} else if (endDate) {
		startSTTC100M.max(new Date(endDate));
	} else {
		endDate = new Date();
		startSTTC100M.max(endDate);
		endSTTC100M.min(endDate);
	}
}
//2. 종료 데이터 피크 이벤트 
function endChangeSTTC100M() {
	var endDate = endSTTC100M.value();
	var startDate = startSTTC100M.value();
	...
	if (endDate) {
		...
		endSTTC100M.min(new Date(startDate));
	} else {
		endDate = new Date();
		startSTTC100M.max(endDate);
		endSTTC100M.min(endDate);
...
//////////////////////////////////////////////////////////////////////////
//그리트 관련 함수
function STTC100M_fnSearchMyCounseling() {
    STTC100M_grdSTTC100M.dataSource.data([]);

    let valid = true;

    Utils.markingRequiredField();
    if (Utils.isNull($('#STTC100M_tenantId').val())) {
    ...
    if(valid) {
        const paramValue = STTC100M_getParamValue();
        STTC100MDataSource.transport.read(paramValue);
    }
}

function STTC100M_getParamValue() {	
	
	var arr = [];
	
	var startDate 		= $("#STTC100M_startDate").val();
	var endDate 		= $("#STTC100M_endDate").val();
	var searchType 		= $("#STTC100M_searchType").val();
	var searchCallType 	= $("#STTC100M_searchCallType").val();
	var statType		= $("#STTC100M_statType").val();
	...
	if(searchType == "TZ" || searchType == ""){
		...
		for(var i=0; i<=STTC100M_getDateDiff(startDate, endDate); i++){
			arr[i] = STTC100M_addDays(startDate, i).toISOString().split('T')[0];
		}
	}
		...
		for(var i=0; i<=STTC100M_getMonthDiff(startDate, endDate); i++){
			arr[i] = STTC100M_addMonth(startDate, i) + "-01";
    ...
    return {
    	serachDateList 		: arr,
        tenantId			: $("#STTC100M_tenantId").val(),
	 	statType			: statType,
 ...
 */
function sttc100_initDateMapping(startDate ,endDate ,searchType ){
	DateMapping = {}
	...
	if(searchType == "TZ" || searchType == ""){
		...
		var fieldTotalCount = STTC100M_getDateDiff(startDate, endDate);
		for(var i=0; i<=fieldTotalCount; i++){
			DateMapping["date" + i] = STTC100M_addDays(startDate, i).toISOString().split('T')[0].replaceAll('-','')
		}
	} else if(searchType == "MN"){
		...
		fieldTotalCount = STTC100M_getMonthDiff(startDate, endDate);

		var endMonYn = false;
		for(var i=0; i<=fieldTotalCount; i++){

			DateMapping["date" + i] =  STTC100M_addMonth(startDate, i).replaceAll('-','')
			if(STTC100M_addMonth(startDate, i) == endDate.substr(0, 7)){
				endMonYn = true;
			}
		}
		if(endMonYn == false){
 ...
 * @작성일      : 2024.04.17
 * @작성자      : shpark
 * @returns {{} | {date0: "0", date1: "0" ... , userId: "" , userNm : "" "totalCount" : "0"}
 */
function sttc100_clearTemplateMap(){
	var templateMap =  Object.assign({}, {"userId" : "" , "userNm" : "" }, DateMapping);
 ...
 * sttc100_clearTemplateMap , sttc100_initDateMapping ,  sttc100_unPack 함수 사용 필수.
 * @작성일      : 2024.04.17
 * @작성자      : shpark
 * @param cnslList
 * @returns {[{userId : 'TEST' , date0 : '5', totalCount : '5'} , {userId : 'TEST2' , date0 : '1', totalCount : '1'} , {userId : '합계' , date0 : '6', totalCount : '6'} ]}
 */
function sttc100_openTree(cnslList){
	console.log("sttc100_openTree")
	// sttc100_initDateMapping 함수에 필요 => date0,date1 은 각각 날짜가 어떻게 되는제 매핑을 위한 map 데이터 생성.
	var searchType 			= $("#STTC100M_searchType").val();
	var startDate 			= $("#STTC100M_startDate").val();
	var endDate 			= $("#STTC100M_endDate").val();
	// 통계유형 구분을 위한 변수.
	...
	tmpList =[]
	DateMapping = sttc100_initDateMapping(startDate,endDate,searchType)

	sttc100_unPack(cnslList)

	var sttcList = []

	var totalMap =  sttc100_clearTemplateMap();
	totalMap.userId = "합계"
	totalMap.userNm = "합계"
		
	tmpList.forEach(function(row){

		var findIndex = sttcList.findIndex((item, index) => item["userId"]== row.userId); // sttcList안에 cnslTypeCd 값이 있나 확인
		...
		if(findIndex >= 0){ // cnslTypeCd 값이 있다면 해당 row에 값 수정
			...
			var map = sttc100_clearTemplateMap();
			map.userId = row.userId;
			map.userNm = row.userNm;
	...
	if(statType == '2'){	//평균 row
		var avgMap  =  sttc100_clearTemplateMap();
		avgMap.userId = "평균"
		avgMap.userNm = "평균"
 ...
 * @param cnslList
 */
function sttc100_unPack(cnslList){
...
function STTC100M_fnResultMyCounseling(data) {
    let cnslList = sttc100_openTree(JSON.parse(data.STTC100M));
    const listLength = cnslList.length
    if (listLength === 0) {
        Utils.alert(STTC100M_langMap.get("fail.common.select"),
            ()=> {return STTC100M_grdSTTC100M.dataSource.data([]);});
        return;
    }
    ...
    STTC100M_grdSTTC100M.dataSource.data(cnslList);
    STTC100M_grdSTTC100M.dataSource.options.schema.data = cnslList;
}
...
//Grid   Height  체크 
function STTC100M_GridResize() {   
	let screenHeight = $(window).height()-200;
	STTC100M_grdSTTC100M.element.find('.k-grid-content').css('height', screenHeight-225);
...
function STTC100M_fnSearchList(){
	...
	//이름 컬럼 추가
	columns.push({ width : 130, field : "userNm", title : STTC100M_langMap.get("STTC100M.gridTitleCnslName"), attributes:{ style:"text-align:left"}, locked: true});
	...
	if(searchType == "TZ" || searchType == ""){
		fieldTotalCount = 24;
		for(var i=0; i<fieldTotalCount; i++){
			columns.push({width : 100, field : "date" + i, title : i + STTC100M_langMap.get("STTC100M.gridTitleHour"), attributes:{style:textAlign}});
		}
	} else if(searchType == "DY"){
		fieldTotalCount = STTC100M_getDateDiff(startDate, endDate);
		for(var i=0; i<=fieldTotalCount; i++){	//시작일과 마지막일 차이를 구하여 for문 실행
			...
			columns.push({width : 100, field : "date" + i, title : STTC100M_addDays(startDate, i).toISOString().split('T')[0], attributes:{style:textAlign}});
		}
		...
		for(var i=0; i<=fieldTotalCount; i++){ //시작월과 마지막월 차이를 구하여 for문 실행
			...
			columns.push({width : 100, field : "date" + i, title : STTC100M_addMonth(startDate, i), attributes:{style:textAlign}});
	...
	//합계 컬럼 추가
	columns.push({ width : 100, field : "totalCount", title : STTC100M_langMap.get("STTC100M.gridTitleTotal"), attributes:{style:textAlign}});
	
	//위에서 정의한 컬럼을 그리드에 설정	
	var grid = $("#grdSTTC100M").data("kendoGrid");
...
//	grid.dataSource.aggregate(aggregate);
	...
	Utils.ajaxCall(
		"/sttc/STTC100SEL01",
		JSON.stringify(STTC100M_getParamValue()),
		STTC100M_fnResultMyCounseling,
		window.kendo.ui.progress($("#grdSTTC100M"), true),
		window.kendo.ui.progress($("#grdSTTC100M"), false)
	)
...
function STTC100M_getDateDiff(d1, d2){
...
function STTC100M_getMonthDiff(d1, d2){
...
function STTC100M_addDays(date, days) {
...
function STTC100M_addMonth(date, month) {
...
function STTC100M_excelExport() {
    ...
    STTC_excelExport(STTC100M_grdSTTC100M, dateString + "_" + STTC100M_langMap.get("STTC100M.title"));
}

function STTC_excelExport(targetGrid, fileName) {
    const pageSize = targetGrid.dataSource.view().length;
    if (pageSize === 0) {
        Utils.alert(STTC100M_langMap.get("STTC100M.validMsg1"));
        return;
    }
...
function STTC100M_changeSerchType(){
	var tmp = $("#STTC100M_searchType").val();
	
	console.log(tmp);
	
	if(tmp == "TZ" || tmp == ""){
		$('#STTC100M_endDate').attr("disabled", true);
	}
	else{
		$('#STTC100M_endDate').attr("disabled", false);
	}
}